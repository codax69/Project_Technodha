...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-cream-100/90 dark:bg-neutral-900/90 backdrop-blur-xl border-b border-cream-200 dark:border-neutral-800 shadow-xs transition-all">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <Link to="/cart">
            <Button
              variant="outline"
              size="sm"
              className="relative rounded-2xl gap-2 px-3 sm:px-3.5 py-1.5 bg-white dark:bg-neutral-900 border-cream-200 dark:border-neutral-800 hover:border-coral-500 shadow-xs hover:shadow-sm text-charcoal-900 dark:text-neutral-100 transition-all font-bold"
            >
          ...
          {/* Desktop Only User Account Dropdown or Auth Buttons */}
      ...
      {/* Mobile Drawer Menu (Contains all other options) */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-cream-200 dark:border-neutral-800 bg-cream-100 dark:bg-neutral-900 px-4 py-4 space-y-3 animate-in slide-in-from-top-3 shadow-xl">
          <Link
            to="/"
          ...
          <div className="pt-2 border-t border-cream-200 dark:border-neutral-800 space-y-3">
            {/* Mobile Theme Toggle Row */}
            <div className="flex items-center justify-between px-3.5 py-2 rounded-xl bg-white/60 dark:bg-neutral-800/60 border border-cream-200 dark:border-neutral-800">
              <span className="text-xs font-bold text-charcoal-700 dark:text-neutral-300">Theme</span>
              <ThemeToggle />
            </div>

            {/* Mobile Account Section / Auth Controls */}
            {user ? (
              <div className="p-3 rounded-2xl bg-white dark:bg-neutral-800 border border-cream-200 dark:border-neutral-700 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-coral-50 dark:bg-coral-950/40 text-coral-500 flex items-center justify-center font-bold text-xs">
                    {user.username ? user.username.substring(0, 2).toUpperCase() : <User className="w-4 h-4" />}
                  </div>
                  <div>
                    <p className="text-xs font-bold text-charcoal-900 dark:text-neutral-100">{user.username}</p>
                    <p className="text-[11px] text-charcoal-700 dark:text-neutral-400">{user.email || 'customer@technodha.com'}</p>
                  </div>
                </div>
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="w-full rounded-xl text-xs font-bold border-rose-200 dark:border-rose-900/40 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 gap-2"
                >
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <Button
                  variant="outline"
                  size="sm"
                  className="rounded-xl text-xs font-semibold text-charcoal-700 dark:text-neutral-300 border-cream-200 dark:border-neutral-700"
                  onClick={() => {
                    setMobileMenuOpen(false);
                    navigate('/login');
                  }}
                ...
                <Button
                  size="sm"
                  className="rounded-xl text-xs font-bold bg-coral-500 hover:bg-coral-600 text-cream-100"
                  onClick={() => {
                    setMobileMenuOpen(false);
                    navigate('/register');
                  }}
